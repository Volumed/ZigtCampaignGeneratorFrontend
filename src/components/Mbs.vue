                ...
                <v-card min-width="100%" min-height="100%" :loading="loading" loader-height="8px">
                    ...
                    <v-card-text>
                        <v-form ref="form" v-model="valid" lazy-validation>
                            <v-checkbox color="secondary" v-model="checkbox" class="mt-4 mb-0" label="Export to dashboard?"></v-checkbox>
                            <v-autocomplete prepend-icon="mdi-domain" class="mt-0 mb-2" color="secondary" item-color="secondary" label="Advertiser name" v-model="advertiser" :items="advertisers" :rules="advertiserRules" required></v-autocomplete>
...
export default {
    ...
    methods: {
        ...
        apiLast () {
             axios.get(process.env.VUE_APP_SECRET + '/mbs')
            .then((response) => {
                for (var i = 0; i < response.data.length; i++) {
        ...
        validate () {
            if (this.$refs.form.validate()) {
                ...
                axios.post(process.env.VUE_APP_SECRET + '/mbs/create',
                {
