                ...
                <v-card min-width="100%" min-height="100%" :loading="loading" loader-height="8px">
                    ...
                    <v-card-text>
                        <v-form ref="form" v-model="valid" lazy-validation>
                            ...
                            <v-select :items="targets" v-model="target" :rules="targetRules" color="secondary" item-color="secondary" class="mb-2" prepend-icon="mdi-target" label="Target"></v-select> 
                            <v-text-field color="secondary" class="mb-2" v-model="adPoNr" prepend-icon="mdi-clipboard-outline" :rules="adPoNrRules" label="Ad management PO-number (optional)"></v-text-field>                                      
                            <v-text-field color="secondary" class="mb-2" v-model="poNr" prepend-icon="mdi-clipboard-outline" :rules="poNrRules" label="PO-number"></v-text-field>
                            <v-text-field color="secondary" class="mb-2" v-model="po2Nr" prepend-icon="mdi-clipboard-outline" :rules="po2NrRules" label="PO-number 2 (optional)"></v-text-field>
                            <v-text-field color="secondary" class="mb-4" v-model="po3Nr" prepend-icon="mdi-clipboard-outline" :rules="po3NrRules" label="PO-number 3 (optional)"></v-text-field>
...
export default {
    data: vm => ({
        ...
        modal: false,
        targets: ['Conversies', 'CTR', 'Engagement', 'Impressies', 'Klikken', 'Uitkijkpercentage', 'Vertoningen', 'Viewability', 'Weergaven'],
        target: '',
        ...
        poNrRules: [
            ...
            v => /^(\d{6}[-]{1})\d{3}$/.test(v) || 'PO-number is not valid.',
        ],
        adPoNrRules: [],
        po2NrRules: [],
        po3NrRules: [],
    ...
    computed: {
      ...
      classObject: function () {
        return {
                success: this.advertiser && this.dateFormatted && this.target && this.name && this.poNr && this.valid && !this.notListed,
                error: !this.valid,
    ...
    watch: {
      ...
      adPoNr (nr) {
          this.generator()
          if (nr.length >= 1) {
            this.adPoNrRules = [v => /^(\d{6}[-]{1})\d{3}$/.test(v) || 'PO-number is not valid.']
          } else {
              this.adPoNrRules = []
          }
      },
      ...
      po2Nr (nr) {
          this.generator()
          if (nr.length >= 1) {
            this.po2NrRules = [v => /^(\d{6}[-]{1})\d{3}$/.test(v) || 'PO-number is not valid.']
          } else {
              this.po2NrRules = []
          }
      },
      po3Nr (nr) {
          this.generator()
          if (nr.length >= 1) {
            this.po3NrRules = [v => /^(\d{6}[-]{1})\d{3}$/.test(v) || 'PO-number is not valid.']
          } else {
              this.po3NrRules = []
          }
      },
    ...
    methods: {
        ...
        apiLast () {
             axios.get(process.env.VUE_APP_SECRET + '/traffic')
            .then((response) => {
                for (var i = 0; i < response.data.length; i++) {
            ...
            if (this.advertiser == 'Advertiser is not in the list') {
               ...
               this.notListed = true
                ...
                if (this.advertiser) {
                    ...
                    if (this.target) {
                        this.campaignName += '_'
                        this.campaignName += this.target
                    }
                    if (this.adPoNr) {
                        this.campaignName += '_AdPO:'
                        this.campaignName += this.adPoNr	
                    }
                    if (this.poNr) {
                        this.campaignName += '_PO:'
                        this.campaignName += this.poNr
                    }
                    if (this.po2Nr) {
                        this.campaignName += '_PO2:'
                        this.campaignName += this.po2Nr
                    }
                    if (this.po3Nr) {
                        this.campaignName += '_PO3:'
                        this.campaignName += this.po3Nr
                    }
                }
            }
        },
        validate () {
            if (this.$refs.form.validate()) {
                ...
                axios.post(process.env.VUE_APP_SECRET + '/traffic/create',
                {
