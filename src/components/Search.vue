                ...
                <v-card min-width="100%" min-height="100%" :loading="loading" loader-height="8px">
                    ...
                    <v-card-text>
                        <v-form ref="form" v-model="valid" lazy-validation>
                            ...
                            <v-checkbox color="secondary" v-model="checkbox" class="mt-4 mb-0" label="Always-on?"></v-checkbox>
                            <v-dialog v-if="dateDisabled" ref="dialog" v-model="modal" :return-value.sync="date" persistent full-width width="290px">
                            ...
                            <v-select :items="countries" :clearable="true" v-model="country" color="secondary" item-color="secondary" class="mb-2" prepend-icon="mdi-flag" label="Target Country"></v-select>                     
                            <v-select :items="groups" :clearable="true" v-model="group" color="secondary" item-color="secondary" class="mb-2" prepend-icon="mdi-account-group" label="Target Group"></v-select>                     
                            <v-select :items="types" :clearable="true" v-model="type" color="secondary" item-color="secondary" class="mb-2" prepend-icon="mdi-hand-pointing-right" label="Type"></v-select>                     
                            <v-text-field color="secondary" class="mb-2" v-model="name" prepend-icon="mdi-pin-outline" :counter="25" :rules="nameRules" label="Enter your unique campaign part"></v-text-field>
                            <v-text-field color="secondary" class="mb-4" v-model="poNr" prepend-icon="mdi-clipboard-outline" :rules="poNrRules" label="PO-number"></v-text-field>
...
export default {
    ...
    computed: {
      ...
      classObject: function () {
            if (this.dateDisabled) {
                return {
                    success: this.advertiser && this.dateFormatted && this.name && this.poNr && this.valid && !this.notListed,
                    error: !this.valid,
                    orange: this.notListed
                }
            } else {
                return {
                    success: this.advertiser && this.name && this.poNr && this.valid && !this.notListed,
                    error: !this.valid,
                    orange: this.notListed
                }
            }
    ...
    methods: {
        ...
        apiLast () {
             axios.get(process.env.VUE_APP_SECRET + '/search')
            .then((response) => {
                for (var i = 0; i < response.data.length; i++) {
               ...
               this.notListed = true
                ...
                if (this.advertiser) {
                    ...
                    if (this.checkbox == true) {
                        this.export = 'Always-On'
                        this.campaignName += '_'
                        this.campaignName += this.export
                    ...
                    if (this.country) {
                        this.campaignName += '_'
                        this.campaignName += this.country
                    }
                    if (this.group) {
                        this.campaignName += '_'
                        this.campaignName += this.group
                    }
                    if (this.type) {
                        this.campaignName += '_'
                        this.campaignName += this.type
                    }
                    if (this.name) {
                        this.campaignName += '_'
        ...
        validate () {
            if (this.$refs.form.validate()) {
                ...
                axios.post(process.env.VUE_APP_SECRET + '/search/create',
                {
