                ...
                <v-card min-width="100%" min-height="100%" :loading="loading" loader-height="8px">
                    ...
                    <v-card-text>
                        <v-form ref="form" v-model="valid" lazy-validation>
                            ...
                            <v-select :items="platforms" v-model="platform" :rules="platformRules" color="secondary" item-color="secondary" class="mb-2" prepend-icon="mdi-flag" label="Which Platform?"></v-select>                     
                            <v-checkbox v-if="instagramStory" color="secondary" v-model="checkbox" class="mt-4 mb-0" label="Instagram Story?"></v-checkbox>
                            <v-select :items="kpis" :rules="kpiRules" v-model="kpi" color="secondary" item-color="secondary" class="mb-2" prepend-icon="mdi-cursor-default-click-outline" label="KPI"></v-select>                                  
...
export default {
    data: vm => ({
        ...
        modal: false,
        platforms: ['Facebook', 'Instagram', 'BestPerforming', 'LinkedIn', 'Snapchat'],
        platform: '',
        platformRules: [v => !!v || 'Platform is required',],
        instagramStory: false,
        kpis: ['Awareness', 'Engagement', 'Conversion', 'Traffic', 'VideoViews', 'Reach'],
        kpiRules: [v => !!v || 'KPI is required',],
        kpi: '',
        name: '',
        nameRules: [
    ...
    computed: {
      ...
      classObject: function () {
        return {
                success: this.advertiser && this.dateFormatted && this.platform && this.kpi && this.name && this.poNr && this.valid && !this.notListed,
                error: !this.valid,
    ...
    methods: {
        ...
        apiLast () {
             axios.get(process.env.VUE_APP_SECRET + '/social')
            .then((response) => {
                for (var i = 0; i < response.data.length; i++) {
            ...
            if (this.advertiser == 'Advertiser is not in the list') {
               ...
               this.notListed = true
                ...
                if (this.advertiser) {
                    ...
                    if (this.platform) {
                        this.campaignName += '_'
                        this.campaignName += this.platform
                    }
                    if (this.platform == 'Instagram') {
                        this.instagramStory = true
        ...
        validate () {
            if (this.$refs.form.validate()) {
                ...
                axios.post(process.env.VUE_APP_SECRET + '/social/create',
                {
